...
## Stack
Python, Django REST Framework, SQLite (local), Claude via a provider interface
(Bedrock is the production swap). Synchronous request flow — no async task queue.
...
4. Routing, state transitions, and all guardrail checks are DETERMINISTIC code — never
   LLM calls.
...
- Unit tests (src/gaps/) verify the DETERMINISTIC spine: given this report, routing
  produces exactly this record. Checked by code, pass/fail.
